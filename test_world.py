...
Everything here runs without a Gazebo. The reader and the writer are XML and
arithmetic; the only thing either needs a CAD library for is the geometry at the
ends of it -- the primitives a world names instead of a mesh, and triangulating
what the writer writes -- and both are stubbed, because what is under test is the
mapping between the two models. Running a world is `test_simulate_gazebo.py`,
which needs no Gazebo either and says why.
...
# name: `gazebo_common` shipped inside the `partcad` wheel until 0.8.80, and a
...
WORLD_EXAMPLE = os.path.join(DATA, "warehouse.world")
...
def dropped_labels():
    ...
    return declaration["import"]["world"].get("dropped") or {}
...
def world_scene(tmp_path, monkeypatch):
    """This repository's test package, with the reader stubbed out.

    A copy rather than the directory itself: two of the tests below rewrite the
    package (`pc convert scene` does that by design), and a test that edits its
    own repository is a test that passes once.
    ...
    config["dependencies"]["sim-gazebo"]["path"] = HERE
...
def test_every_counter_the_reader_keeps_has_a_wording():
    ...
    The reader counts and `partcad.yaml` names, so the two are only ever right
    together: a counter added to the script and not to the declaration reports
    itself as 'joint_kinematics: 3' to a user.
    """
...
def test_a_scene_does_not_convert_to_a_format_that_describes_one_product(world_scene):
    ...
    Which is a claim the declaration makes and the core holds it to, rather than
    a list of formats anybody keeps: this package's own says `kinds: [scene]`
...
def test_converting_a_scene_that_is_already_that_format_does_nothing(world_scene):
    ...
    project, _scene = world_scene
    assert convert_scene_action(project, "warehouse", "sim-gazebo:world") is None
...
def test_import_scene_refuses_to_overwrite_an_existing_scene(world_scene):
    ...
    with pytest.raises(ValueError, match="already has a scene named 'warehouse'"):
        import_scene_action(project, "sim-gazebo:world", os.path.join(project.config_dir, "warehouse.world"), {})
